                    ...
                    <form action="<?= URLROOT ?>/admin/Products/edit/<?= htmlspecialchars($p_id) ?>" method="POST" enctype="multipart/form-data">
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label for="name" class="fw-bold">Tên sản phẩm <span class="text-danger">(*)</span></label>
                                <input type="text" class="form-control" id="name" name="name" value="<?= htmlspecialchars($p_name) ?>" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label for="sku" class="fw-bold">Mã SKU <span class="text-danger">(*)</span></label>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                ...
                                <select class="form-control" id="cateId" name="cateId" required onchange="toggleNewCategory()" style="height: auto;">
                                    ...
                                    <?php endforeach; ?>
                                    <option value="new" class="fw-bold text-primary">+ Khác (Thêm danh mục mới)</option>
                                </select>
                            ...
                            <div class="col-md-4 mb-3">
                                <label for="price" class="fw-bold">Giá (VNĐ) <span class="text-danger">(*)</span></label>
                                <input type="text" class="form-control" id="price" name="price" value="<?= number_format($p_price, 0, '', '.') ?>" required oninput="formatPrice(this)">
                            </div>
                            <div class="col-md-4 mb-3">
                                <label for="stock_quantity" class="fw-bold">Số lượng trong kho</label>
                        ...
                        <div class="row">
                            <div class="col-md-4 mb-3">
                                <label for="color" class="fw-bold">Màu sắc</label>
                                <select class="form-control" id="color_select" name="color_select" onchange="toggleCustomColor()" style="height: auto;">
                                    ...
                                    <?php endforeach; ?>
                                    <option value="other" class="fw-bold text-primary" <?= $isCustomColor ? 'selected' : '' ?>>+ Khác...</option>
                                </select>
                            ...
                            <div class="col-md-4 mb-3">
                                <label for="size" class="fw-bold">Kích cỡ (Size)</label>
                                <div class="mb-2">
                                    <?php foreach ($stdSizes as $s): ?>
                                    <div class="form-check form-check-inline">
                                        <input type="checkbox" class="form-check-input" id="size_<?= $s ?>" name="sizes[]" value="<?= $s ?>" <?= in_array($s, $p_sizes_arr) ? 'checked' : '' ?>>
                        ...
                        <div class="mb-3">
                            <label for="description" class="fw-bold">Mô tả sản phẩm</label>
...
<script>
    ...
    document.querySelectorAll('.image-delete-cb').forEach(function(cb) {
        cb.addEventListener('change', function() {
            ...
            if (this.checked) {
                ...
                // Chặn không cho Hủy xóa nếu tổng số ảnh vượt quá 5
                let proposedCount = totalExisting - deletedUrls.length + 1 + dataTransfer.files.length;
                if (proposedCount > 5) {
    ...
    function renderPreviews() {
        ...
        for (let i = 0; i < dataTransfer.files.length; i++) {
            ...
            div.innerHTML = `
                <img src="" class="w-100 h-100 preview-img" style="object-fit: contain;">
                <div class="position-absolute" style="top: -8px; right: -8px;">
            ...
            reader.addEventListener("load", function() {
                let img = div.querySelector('.preview-img');
                img.src = this.result;
            });
    ...
    function updateValidation() {
        ...
        let total = totalExisting - deletedUrls.length + count;
        let countSpan = document.getElementById('image-count');
