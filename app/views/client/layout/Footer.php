...
$siteEmail = (string)($content['site_email'] ?? 'contact@aurelia.vn');
$siteAddress = (string)($content['site_address'] ?? '123 Duong Le Loi, Quan 1, TP. Ho Chi Minh');
$workingHours = (string)($content['contact_working_hours'] ?? '8:00 - 21:00, T2 - CN');
...
<footer class="bg-slate-900 text-amber-50/80">
	<div class="container mx-auto px-4 py-12 md:py-16">
		<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
			<div>
				<h3 class="text-2xl font-bold text-amber-50 mb-4 tracking-wide"><?= htmlspecialchars($siteBrand, ENT_QUOTES, 'UTF-8') ?></h3>
			...
			<div>
				...
				<div class="space-y-3 text-sm text-amber-50/60">
					<p class="flex items-start gap-2">
						...
						<?= htmlspecialchars($siteAddress, ENT_QUOTES, 'UTF-8') ?>
					</p>
					<p class="flex items-center gap-2">
						...
						<?= htmlspecialchars($sitePhone, ENT_QUOTES, 'UTF-8') ?>
					</p>
					<p class="flex items-center gap-2">
						<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0 text-amber-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
						...
						<?= htmlspecialchars($siteEmail, ENT_QUOTES, 'UTF-8') ?>
					</p>
					<p class="flex items-center gap-2">
						<svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0 text-amber-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
						...
						<?= htmlspecialchars($workingHours, ENT_QUOTES, 'UTF-8') ?>
					</p>
				</div>
			</div>
